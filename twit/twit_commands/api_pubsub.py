...
from twit.twit_api.api_command_handler import calc_play_time, get_game_data
...
def run_pubsub_point_redemption(reward, reward_cost, player):
    ...
    # get wit total to pass in update_player to determine half-wit (calling get_game_data here is 1x, versus 2or3x)
    ...
    update_player(player, (update_play_time, reward_cost, wit_total), update_type='TIME')
    
    # call it again to get the updated wit for the guest accounts (keeps from adding 0.5 to bots)
    game_data = get_game_data()
    wit_total = game_data['WIT']['total'].to_decimal()
    progress.w(f'{player} redeemed {reward_cost} channel points')
    ...
    if guest_data_count in {1}:
        ...
        update_player(guest_one, (update_play_active, reward_cost, wit_total), update_type='TIME')
    ...
    if guest_data_count in {2}:
        ...
        update_play_active = calc_play_time(guest_play_active, guest_play_until)
        update_player(guest_two, (update_play_active, reward_cost, wit_total), update_type='TIME')
