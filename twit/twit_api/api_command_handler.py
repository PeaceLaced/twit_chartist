...
def toDecimal128(decimal_data: Decimal) -> Decimal128:
    ''' convert a Decimal to Decimal128'''
    
    with localcontext(create_decimal128_context()) as ctx:
        return Decimal128(ctx.create_decimal(decimal_data))

def get_player_count(count_type='total') -> int:
    ''' count total players or current players'''
...
def is_SUPERUSER(player: str) -> bool:
    ''' check if player is on the super user list'''
    
    if player not in SUPER_USER:
        return False
    return True

def is_valid_player(valid_player: str) -> bool:
    ''' check if player is a valid twitch name'''
...
def is_active_player(active_player: str) -> bool | dict:
...
def update_active_players(current_wit_value: Decimal) -> tuple[Decimal, int]:
    ''' 
...
def get_player_data(player: str, query_type=False) -> bool | dict:
    ''' query and return player data, add player if they do not exist, return False if invalid player'''
...
def calc_play_time(play_active: bool, play_until: datetime) -> datetime:
    ''' add 20 minutes from now or current play_until time'''
    if not play_active:
...
def update_player(player: str, update_data: str | tuple | Decimal , /, update_type: str) -> str | list | Decimal:
    ''' update player depends on update_type'''
    ...
    with MongoClient(tz_aware=True) as mongo_client:
        ...
        if update_type in {'TIME'}:
            ...
            total_wit = update_data[2]
            
            # initial gets 0.5 wit to player, system, and halfwit switched on
            if total_wit == Decimal(0.0):
                update_wit = Decimal(0.5)
                write_update_wit = toDecimal128(update_wit)
                mongo_client['twit_chartist']['twit_champ'].find_one_and_update(
                    {f'{player}' : {'$exists':True}},
                    {'$set' : {f'{player}.halfwit':True},
                     '$inc' : {f'{player}.WIT':write_update_wit}})
                update_game(None, update_wit, update_type='SYSTEM-WIT')
