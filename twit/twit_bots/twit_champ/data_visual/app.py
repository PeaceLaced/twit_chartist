...
def generate_data():
    ...
    with MongoClient(tz_aware=True) as mongo_client:
        data_from_db = mongo_client['twit_chartist']['twit_chart'].find_one()
        wit_modifier = data_from_db['WIT']['modifier'].to_decimal()
        wit_total = data_from_db['WIT']['total'].to_decimal()
        wit_value =  wit_modifier / wit_total
    
    return datetime.now(pytz.timezone('US/Eastern')) - timedelta(since_start), wit_value
...
app.run_server(debug=True, use_reloader=False, dev_tools_ui=False, port=8051)
#use_reloader=False, dev_tools_ui=False)